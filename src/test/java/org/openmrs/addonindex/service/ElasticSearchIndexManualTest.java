...
public class ElasticSearchIndexManualTest {
	...
	public void testIndex() throws IOException {
		...
		Collection<AddOnInfoSummary> results = elasticSearchIndex.search(List.of(AddOnType.OMOD), "testing", null);
	...
	public void testGetByModulePackage() throws IOException {
		SearchResponse response = client.search(
		    new SearchRequest(AddOnInfoAndVersions.ES_INDEX).source(
		        new SearchSourceBuilder().size(1).query(QueryBuilders.matchQuery("modulePackage", "testing-mod"))),
		    RequestOptions.DEFAULT);
		
		SearchResponse responseNotFound = client.search(
		    new SearchRequest(AddOnInfoAndVersions.ES_INDEX)
		            .source(new SearchSourceBuilder().size(1).query(QueryBuilders.matchQuery("modulePackage", "fake-mod"))),
		    RequestOptions.DEFAULT);
		
		List<AddOnInfoAndVersions> searchResponse = Arrays.stream(response.getHits().getHits()).filter(SearchHit::hasSource)
	...
	public void testTag() throws IOException {
		SearchResponse response = client.search(new SearchRequest(AddOnInfoAndVersions.ES_INDEX).source(
		    new SearchSourceBuilder().query(QueryBuilders.matchQuery("tags", "form-entry"))), RequestOptions.DEFAULT);
