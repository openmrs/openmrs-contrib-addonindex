...
public class FetchCoreVersionsList {
	...
	@Autowired
	public FetchCoreVersionsList(RestTemplateBuilder restTemplateBuilder, ObjectMapper objectMapper,
	...
	@Scheduled(initialDelayString = "${scheduler.fetch_core_versions_list.initial_delay}", fixedDelayString = "${scheduler.fetch_core_versions_list.period}")
	public void fetchCoreVersionsList() throws Exception {
		...
		if (versionlist.size() > 0) {
			...
			for (ArtifactoryFolderInfoChild candidateVersion : allVersions) {
				if (candidateVersion.getFolder()
				        && !stringContainsItemFromList(candidateVersion.getUri(), STRINGS_TO_EXCLUDE)) {
