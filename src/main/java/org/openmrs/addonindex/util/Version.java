 ...
 */
/*
 * This implementation is taken from Maven's ComparableVersion class
 * as suggested by https://stackoverflow.com/a/26745775
 *
 * This is the version found in Maven 3.8.1
 */
...
public class Version implements Comparable<Version> {
	...
	private static class StringItem implements Item {
		...
		/**
		 * Returns a comparable value for a qualifier. This method takes into account the ordering of known
		 * qualifiers then unknown qualifiers with lexical ordering. just returning an Integer with the
		 * index here is faster, but requires a lot of if/then/else to check for -1 or QUALIFIERS.size and
		 * then resort to lexical ordering. Most comparisons are decided by the first character, so this is
	...
	public static void main(String... args) {
		...
		for (String version : args) {
			Version c = new Version(version);
			
			if (prev != null) {
				int compare = prev.compareTo(c);
				System.out
				        .println("   " + prev + ' ' + ((compare == 0) ? "==" : ((compare < 0) ? "<" : ">")) + ' ' + version);
			}
